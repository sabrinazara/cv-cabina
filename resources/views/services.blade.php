...
@section('content')
    ...
    <section class="py-16 bg-white">
        <div class="container mx-auto px-6 text-center">
            <h2 class="text-4xl font-bold mb-2">My <span class="text-teal-600">Services</span></h2>
        </div>
    </section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @forelse ($services as $service)
                    <div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition duration-300">
                        <i class="{{ $service->icon ?? 'fas fa-cogs' }} text-5xl text-teal-600 mb-4"></i>
                        <h4 class="text-xl font-semibold mb-3">{{ $service->title }}</h4>
                        <p class="text-gray-600 text-sm">{{ $service->description }}</p>
                    </div>
                @empty
                    <p class="md:col-span-3 text-gray-500">Belum ada layanan yang ditambahkan. Silakan isi melalui CMS.</p>
                @endforelse
            </div>
        </div>
    </section>
